...
public class EncryptionClassDrei
   {
    ...
    EncryptionClassDrei( EncryptionClassDrei copied )
       {
        NodeClass thisWkgRef, copiedWkgRef;
        ...
        if( copied.headRef != null )
           {
            ...
            while( copiedWkgRef != null )
               {
                thisWkgRef.nextRef = new NodeClass( copiedWkgRef );
                
                thisWkgRef = thisWkgRef.nextRef;
    ...
    public char decodeCharacter( long encodedLong )
       {
        long wkgInt = encodedLong / TWO_DIGIT_OFFSET;
        long firstDigit = wkgInt % ONE_DIGIT_OFFSET;
        long secondDigit, thirdDigit, resultVal;
        
        wkgInt /= THREE_DIGIT_OFFSET;
        
        secondDigit = wkgInt % ONE_DIGIT_OFFSET;
        
        wkgInt /= THREE_DIGIT_OFFSET;
        
        thirdDigit = wkgInt;
        
        resultVal = thirdDigit;
        
        resultVal += secondDigit * ONE_DIGIT_OFFSET;
        
        resultVal += firstDigit * TWO_DIGIT_OFFSET;
        
        return (char)resultVal;
       }
        ...
        while( wkgRef != null )
           {
            ...
            decodeLong = wkgRef.encryptedVal;
            
            decodeChar = decodeCharacter( decodeLong );
            
            System.out.format( "%10d(\'%c\')", decodeLong, decodeChar );
     ...
     * <p>
     * Note: Must protect against creating random value with CARET_CHAR
     * <p>
     * Dependencies: getRandBetween
     * 
    ...
    public long generateNineDigitRandom()
       {
        ...
        long nineDigitRand = getRandBetween( THREE_DIGIT_LOW, THREE_DIGIT_HIGH );

        nineDigitRand *= THREE_DIGIT_OFFSET;
        
        nineDigitRand += getRandBetween( THREE_DIGIT_LOW, THREE_DIGIT_HIGH );
        
        nineDigitRand *= THREE_DIGIT_OFFSET;
        
        nineDigitRand += getRandBetween( THREE_DIGIT_LOW, THREE_DIGIT_HIGH );

        tempNode = new NodeClass( nineDigitRand );
        
        if( isCharacter( tempNode, CARET_CHAR ) ) 
           {
            return generateNineDigitRandom();   
           }
        
        return nineDigitRand;
       }
    ...
    public boolean isCharacter( NodeClass testRef, char testChar )
       {
        char refChar = decodeCharacter( testRef.encryptedVal );
        
        return refChar == testChar;
       }
